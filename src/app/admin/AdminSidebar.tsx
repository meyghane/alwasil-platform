...
const groups = [
  ...
  { label: 'Qualité', links: [['/admin/fraicheur', 'Fraîcheur', Gauge], ['/admin/couverture', 'Couverture', Archive], ['/admin/historique', 'Historique', History]] },
...
export default function AdminSidebar() {
  return <aside style={{ width: 238, flexShrink: 0, background: '#080808', color: 'white', minHeight: 'calc(100vh - 74px)', padding: '1.25rem .8rem', position: 'sticky', top: 0, alignSelf: 'flex-start' }}>
    ...
    {groups.map(group => <div key={group.label} style={{ marginBottom: '1.35rem' }}><div style={{ color: 'rgba(255,255,255,.45)', textTransform: 'uppercase', letterSpacing: '.1em', fontSize: 10, fontWeight: 800, padding: '0 .75rem', marginBottom: '.45rem' }}>{group.label}</div>{group.links.map(([href, label, Icon]) => <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: '.6rem', color: 'rgba(255,255,255,.78)', textDecoration: 'none', padding: '.62rem .75rem', borderRadius: 8, fontSize: 13, marginBottom: 2 }}><Icon size={15} strokeWidth={1.8} />{label}</Link>)}</div>)}

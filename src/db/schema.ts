...
export const leads = pgTable('leads', {
  id: uuid('id').primaryKey().defaultRandom(), ticketReference: text('ticket_reference').unique(), offerId: text('offer_id'), partnerId: text('partner_id'), status: leadStatusEnum('status').notNull().default('new'), name: text('name').notNull(), email: text('email').notNull(), phone: text('phone'), travelType: text('travel_type').notNull(), qualification: jsonb('qualification').$type<Record<string, unknown>>().default({}), source: text('source'), utm: jsonb('utm').$type<Record<string, string>>().default({}), consentFollowUp: boolean('consent_follow_up').notNull().default(false), createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
});

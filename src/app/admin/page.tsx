...
export default async function AdminPage() {
 ...
 <div style={{ marginBottom: '2rem' }}>
  ...
  <div className="admin-quality-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '.75rem' }}>
   {[['/admin/soumissions','Nouvelles fiches','Valider ou archiver'],['/admin/base','Gestion des fiches','Rechercher, modifier, archiver et consulter l’historique'],['/admin/signalements','Signalements','Traiter les retours visiteurs'],['/admin/couverture','Couverture','Repérer les départements incomplets'],['/admin/fraicheur','Fraîcheur','Voir les fiches à vérifier'],['/admin/auto','Automatisations','Suivre les exécutions'],['/admin/journal','Journal','Comprendre les erreurs automatiques']].map(([href,title,desc]) => <Link key={href} href={href} style={{ textDecoration:'none', background:'white', border:'1px solid #e2d7f5', borderRadius:12, padding:'1rem' }}><div style={{fontWeight:800,color:DARK,fontSize:'.85rem'}}>{title}</div><div style={{fontSize:'.72rem',color:'#59565f',marginTop:'.35rem'}}>{desc}</div></Link>)}
  </div>
 </div>

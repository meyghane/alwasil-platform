...
export default function DatabaseClient() {
  ...
  useEffect(() => { load(); }, []);
  async function action(id: string, actionName: 'archive' | 'delete' | 'reverify') {
  ...
  return <div>
    <div style={{ display: 'flex', gap: 8, margin: '1.5rem 0', flexWrap: 'wrap' }}><input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') load(); }} placeholder="Rechercher une fiche, une ville..." style={{ flex: 1, minWidth: 240, padding: 12, border: '1px solid #ddd6fe', borderRadius: 8 }} /><select value={status} onChange={e => { setStatus(e.target.value); setTimeout(load, 0); }} style={{ padding: 12, border: '1px solid #ddd6fe', borderRadius: 8 }}><option value="">Tous les statuts</option><option value="approved">Publiées</option><option value="pending">À vérifier</option><option value="rejected">Rejetées</option><option value="expired">Archivées</option></select><button onClick={load} style={{ padding: '0 16px', border: 0, borderRadius: 8, background: '#7652CA', color: 'white', fontWeight: 700 }}>Rechercher</button></div>
    {loading ? <p>Chargement...</p> : <div style={{ display: 'grid', gap: 10 }}>{items.map(item => <article key={item.id} style={{ background: 'white', border: '1px solid #e2d7f5', borderRadius: 10, padding: 14 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div style={{ minWidth: 0 }}><strong>{item.title}</strong><div style={{ color: '#6b7280', fontSize: 13 }}>{item.category} | {item.status} | {item.city || 'Ville inconnue'}</div>{item.description && <p style={{ color: '#59565f', fontSize: 13, margin: '8px 0 0' }}>{item.description}</p>}{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noreferrer" style={{ color: '#7652CA', fontSize: 12 }}>Voir la source</a>}</div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'flex-start' }}><button onClick={() => edit(item)}>Modifier</button><button onClick={() => action(item.id, 'reverify')}>Re vérifier</button><button onClick={() => action(item.id, 'archive')}>Archiver</button><button onClick={() => action(item.id, 'delete')}>Supprimer</button></div></div></article>)}</div>}

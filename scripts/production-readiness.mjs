...
const migrations = ['0005_reports.sql', '0008_moderation_history.sql', '0009_moderation_undo.sql', '0010_audience_events.sql', '0011_lead_ticket_reference.sql', '0012_telegram_delivery.sql', '0013_partner_verification.sql', '0014_agents.sql'];
try {
  ...
  const requirements = { moderation_log: ['previous_status','new_status'], reports: ['id'], audience_events: ['id'], telegram_deliveries: ['dedupe_key','result'], partners: ['source_url','verified_at'], leads: ['ticket_reference'], agent_sources: ['id','trust'], agent_runs: ['id','run_key'], agent_item_history: ['before_snapshot','rollback_until'], agent_source_history: ['before_snapshot','after_snapshot'] };

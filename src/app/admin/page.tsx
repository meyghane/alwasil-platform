...
export default async function AdminPage() {
 ...
 <div style={{ marginBottom: '2rem' }}>
  ...
  <div className="admin-quality-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '.75rem' }}>
   {[['/admin/soumissions','À traiter','Valider, rejeter ou archiver les fiches'],['/admin/base','Toutes les fiches','Rechercher et gérer les fiches publiées ou archivées'],['/admin/signalements','Signalements','Traiter les retours visiteurs'],['/admin/fraicheur','Fiches à revérifier','Contrôler les liens, dates et informations'],['/admin/couverture','Couverture géographique','Repérer les départements incomplets'],['/admin/historique','Historique des actions','Voir qui a fait quoi et annuler une action'],['/admin/auto','Automatisations','Suivre les collectes automatiques']].map(([href,title,desc]) => <Link key={href} href={href} style={{ textDecoration:'none', background:'white', border:'1px solid #e2d7f5', borderRadius:12, padding:'1rem' }}><div style={{fontWeight:800,color:DARK,fontSize:'.85rem'}}>{title}</div><div style={{fontSize:'.72rem',color:'#59565f',marginTop:'.35rem'}}>{desc}</div></Link>)}
  </div>
 </div>

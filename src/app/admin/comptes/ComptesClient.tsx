...
export default function ComptesClient() {
 ...
 {showForm && (
 <div style={{ backgroundColor: 'white', borderRadius: '20px', border: '1px solid #f0ebfa', padding: '1.75rem', marginBottom: '2rem', boxShadow: '0 4px 24px rgba(124,58,237,0.08)' }}>
 ...
 style={{ width: '100%', padding: '0.7rem 0.875rem', border: '2px solid #f0ebfa', borderRadius: '8px', fontSize: '0.9rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'Poppins, sans-serif' }} />
 </div>
 <div>
 ...
 style={{ padding: '0.5rem 1rem', borderRadius: '8px', border: `2px solid ${form.role === role ? VIOLET : '#f0ebfa'}`, backgroundColor: form.role === role ? VIOLET : 'white', color: form.role === role ? 'white' : '#374151', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.35rem', fontFamily: 'Poppins, sans-serif' }}>
 ...
 style={{ padding: '0.75rem 1.25rem', backgroundColor: '#f0ebfa', color: '#6b7280', border: '1px solid #f0ebfa', borderRadius: '10px', fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}>
 Annuler
 </button>
 </div>
 ...
 {inviteLink && (
 <div style={{ marginTop: '1.25rem', padding: '1rem', backgroundColor: '#f0fdf4', border: '1px solid #e2d7f5', borderRadius: '12px' }}>
 <div style={{ fontWeight: 700, fontSize: '0.82rem', color: '#543398', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
 Invitation envoyée ! Lien de secours si l'email n'arrive pas :
 </div>
 <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
 <code style={{ flex: 1, fontSize: '0.72rem', color: '#374151', backgroundColor: 'white', padding: '0.5rem 0.75rem', borderRadius: '8px', border: '1px solid #f0ebfa', wordBreak: 'break-all', lineHeight: 1.4 }}>
 {inviteLink}
 </code>
 <button onClick={copyLink}
 style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.5rem 0.875rem', backgroundColor: copied ? '#7652CA' : VIOLET, color: 'white', border: 'none', borderRadius: '8px', fontWeight: 700, fontSize: '0.75rem', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, fontFamily: 'Poppins, sans-serif' }}>
 ...
 <div style={{ textAlign: 'center', padding: '3rem', backgroundColor: 'white', borderRadius: '16px', border: '1px solid #f0ebfa', color: '#9ca3af' }}>
 <UserPlus size={36} color="#e2d7f5" strokeWidth={1.4} style={{ display: 'block', margin: '0 auto 1rem' }} />
 ...
 return (
 <div key={compte.id} style={{ backgroundColor: 'white', borderRadius: '14px', border: `1px solid ${compte.actif ? '#f0ebfa' : '#f3f4f6'}`, overflow: 'hidden', opacity: compte.actif ? 1 : 0.65 }}>

 {/* Ligne principale */}
 <div style={{ padding: '1.1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
 <div style={{ width: 44, height: 44, borderRadius: '12px', background: compte.role === 'admin' ? 'linear-gradient(135deg, #7652CA, #543398)' : 'linear-gradient(135deg, #6540b5, #543398)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: 'white', fontSize: '1.1rem', flexShrink: 0, fontFamily: 'Poppins, sans-serif' }}>
 {(compte.name || compte.email).charAt(0).toUpperCase()}
 </div>

 <div style={{ flex: 1, minWidth: 0 }}>
 <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#080808', fontFamily: 'Poppins, sans-serif' }}>{compte.name || '(en attente)'}</div>
 <div style={{ fontSize: '0.75rem', color: '#9ca3af' }}>{compte.email}</div>
 <div style={{ display: 'flex', gap: '0.35rem', marginTop: '0.35rem', flexWrap: 'wrap' }}>
 <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '2px 7px', borderRadius: '4px', backgroundColor: compte.role === 'admin' ? '#f0ebfa' : '#f0ebfa', color: compte.role === 'admin' ? VIOLET : '#6540b5' }}>
 {compte.role === 'admin' ? ' Admin' : ' Modo'}
 </span>
 {compte.actif
 ? <span style={{ fontSize: '0.65rem', padding: '2px 7px', borderRadius: '4px', backgroundColor: '#f0fdf4', color: '#7652CA' }}>Actif</span>
 ...
 style={{ background: 'none', border: 'none', cursor: 'pointer', color: compte.actif ? '#7652CA' : '#9ca3af', display: 'flex', alignItems: 'center' }}>
 ...
 style={{ background: 'none', border: '1px solid #f0ebfa', borderRadius: '8px', cursor: 'pointer', padding: '0.35rem', display: 'flex', alignItems: 'center', color: VIOLET }}>
 ...
 {/* Panel permissions */}
 {isExpanded && (
 <div style={{ padding: '0 1.25rem 1.25rem', borderTop: '1px solid #f0ebfa' }}>

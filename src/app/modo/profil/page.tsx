...
function getLevel(h: number) {
 ...
 if (h >= 500) return { label: 'Bâtisseur', ar: 'بانٍ', color: '#7652CA', next: 1000, progress: Math.round(((h - 500) / 500) * 100) };
 if (h >= 200) return { label: 'Nāfi\'', ar: 'نافع', color: '#7652CA', next: 500, progress: Math.round(((h - 200) / 300) * 100) };
 if (h >= 75) return { label: 'Mousāhim', ar: 'مساهم', color: '#6540b5', next: 200, progress: Math.round(((h - 75) / 125) * 100) };

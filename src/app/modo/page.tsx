...
function getLevel(h: number) {
 ...
 if (h >= 500) return { label: 'Bâtisseur', ar: 'بانٍ', color: '#7652CA', next: 1000, min: 500, progress: Math.round(((h-500)/500)*100) };
 if (h >= 200) return { label: 'Nāfi\'', ar: 'نافع', color: '#7652CA', next: 500, min: 200, progress: Math.round(((h-200)/300)*100) };
 if (h >= 75) return { label: 'Mousāhim', ar: 'مساهم', color: '#6540b5', next: 200, min: 75, progress: Math.round(((h-75)/125)*100) };
...
export default async function ModoDashboard() {
 ...
 <Shield size={18} color="#c9b6ec" strokeWidth={1.8} />
 </div>
 <div>
 ...
 <div style={{ backgroundColor: 'white', borderRadius: '16px', border: '1px solid #f0ebfa', padding: '1rem 1.25rem', minWidth: 220, boxShadow: '0 4px 16px rgba(124,58,237,0.08)', cursor: 'pointer' }}>
 <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
 {/* Avatar */}
 <div style={{ width: 36, height: 36, borderRadius: '10px', background: 'linear-gradient(135deg, #7652CA, #543398)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: 'white', fontSize: '1rem', flexShrink: 0 }}>
 {session.name.charAt(0).toUpperCase()}
 </div>
 <div style={{ flex: 1, minWidth: 0 }}>
 <div style={{ fontWeight: 700, fontSize: '0.82rem', color: '#080808', fontFamily: 'Poppins, sans-serif', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{session.name}</div>
 ...
 <div style={{ background: 'linear-gradient(135deg, #543398, #3b0764)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', boxShadow: '0 8px 24px rgba(76,29,149,0.4)', cursor: 'pointer', height: '100%', boxSizing: 'border-box' }}>
 ...
 <Sparkles size={22} color="#c9b6ec" strokeWidth={1.6} />
 </div>
 <div>
 ...
 <div style={{ background: 'linear-gradient(135deg, #7652CA, #543398)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', boxShadow: '0 8px 24px rgba(124,58,237,0.3)', cursor: 'pointer', height: '100%', boxSizing: 'border-box' }}>
 ...
 Ajouter une fiche manuellement <span style={{ color: '#c9b6ec', fontWeight: 600 }}>+10 </span>
 </h2>
 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.875rem' }}>
 {allowedCats.map(cat => (
 <Link key={cat.key} href={`/modo/ajouter/${cat.key}`} style={{ textDecoration: 'none' }}>
 <div style={{ backgroundColor: 'white', borderRadius: '14px', border: '1px solid #f0ebfa', padding: '1.1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(109,40,217,0.06)', cursor: 'pointer' }}>
 <div>
 <div style={{ fontWeight: 700, fontSize: '0.875rem', color: '#080808', fontFamily: 'Poppins, sans-serif' }}>{cat.label}</div>

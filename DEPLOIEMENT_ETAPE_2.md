...
La migration complète reste ouverte. `/api/contact` envoie toujours à Make et Telegram en complément de Resend, et les comptes, mots de passe et l'historique du chat dépendent de Sheets. `/api/mosques` lit encore Sheets pour les fiches historiques mais affiche aussi les mosquées approuvées dans Neon. Ces données ne peuvent pas être coupées sans migration et vérification. Les anciennes fiches en attente dans Sheets restent visibles dans `/api/admin/soumissions` et peuvent toujours être traitées. Ne pas supprimer Apps Script ou ses secrets avant reprise de ces parcours.
